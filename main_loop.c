...
/*   Updated: 2021/11/04 00:09:14 by dogata           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main_loop(t_game *game)
{
	...
	if (game->escape)
		g_escape_count++;
	if (g_loop_called_times == 8)
	{
		g_loop_called_times = 0;
		if (game->move)
			game->pl.player_tex++;
	}
	if (game->pl.player_tex == 4)
		game->pl.player_tex = 0;
	draw_base_image(game);
	draw_image(game, 'C', game->tex.sprite);
	if (game->pl.move_draw_count == 0)
		game->move = false;
	draw_player_image(game, game->tex.player[game->pl.player_tex + game->pl.direction]);
